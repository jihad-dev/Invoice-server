...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.5ipn6sc.mongodb.net/InvoiceFinalDB?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  ...
  try {
    ...
    // proforma post route //
    app.post("/proforma", async (req, res) => {
      const info = req.body;
      const result = await ProformaCollection.insertOne(info);
      console.log(result);
      res.send(result);
    });

    // proforma get all info //
    app.get("/proforma", async (req, res) => {
      const query = {};
      const result = await ProformaCollection.find(query).toArray();
      res.send(result);
    });

       // specefic data load proforma page //

       app.get("/proforma/:id", async (req, res) => {
        const query = { _id: new ObjectId(req.params.id) };
        const result = await ProformaCollection.findOne(query);
        res.send(result);
      });
